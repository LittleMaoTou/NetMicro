...
using NetMicro.Data.Provider.ExpressionResolve;
...
namespace NetMicro.Data.Provider.SqlQueryable.Internal
{
    internal class QueryBuilder
    {
        ...
        public string UpdateSqlBuild(out IQueryParameters parameters)
        {
            ...
            sqlBuilder.AppendFormat("UPDATE {0} SET ", _sqlAdapter.GetTableName(tableName));
        ...
        public string DeleteSqlBuild(out IQueryParameters parameters)
        {
            ...
            sqlBuilder.AppendFormat("DELETE FROM {0} ", _sqlAdapter.GetTableName(tableName));
        ...
        public string SoftDeleteSqlBuild(out IQueryParameters parameters)
        {
            ...
            var sqlBuilder = new StringBuilder($"UPDATE {_sqlAdapter.GetTableName(tableName)} SET ");
            sqlBuilder.AppendFormat("{0}={1},", _sqlAdapter.AppendQuote(deletedColumnName), "1");
        ...
        private void ResolveFrom(StringBuilder sqlBuilder, IQueryParameters parameters)
        {
            ...
            {
                sqlBuilder.AppendFormat(" {0} ", _sqlAdapter.GetTableName(first.TableName));
            ...
            sqlBuilder.AppendFormat(" {0} AS {1} ", _sqlAdapter.GetTableName(first.TableName), _sqlAdapter.AppendQuote(first.Alias));
            //附加NOLOCK特性
            if (_sqlAdapter.SqlDialect == SqlDialect.SqlServer && first.NoLock)
            {
                sqlBuilder.Append("WITH (NOLOCK) ");
            ...
            for (var i = 1; i < _queryBody.JoinDescriptors.Count; i++)
            {
                ...
                sqlBuilder.AppendFormat("JOIN {0} AS {1} ", _sqlAdapter.GetTableName(descriptor.TableName), _sqlAdapter.AppendQuote(descriptor.Alias));
                //附加NOLOCK特性
                if (_sqlAdapter.SqlDialect == SqlDialect.SqlServer && first.NoLock)
                {
                    sqlBuilder.Append("WITH (NOLOCK) ");
